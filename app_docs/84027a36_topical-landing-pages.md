...
| `app/controllers/public/discovery_controller.rb` | Added `SITEMAP_CACHE_TTL`, `landing_sitemap_urls` private method, and `landing_sitemap_rows_for` helper. It emits only topic slugs accepted by the topic controller, bodies with current meetings, and years accepted by the year controller, so the sitemap never advertises a rejected landing route. |
| `app/models/public/landing_page_eligibility.rb` | Shared topic, body, and year eligibility checks used by both the landing-page controllers and sitemap generation. |
| `app/controllers/concerns/public_records_in_cached_order.rb` | Shared helper for restoring cached ID lists to their requested order without duplicating it across landing-page controllers. |
