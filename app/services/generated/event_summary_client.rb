...
module Generated
  # EventSummaryClient calls an OpenAI-compatible /chat/completions endpoint and
  # returns the parsed event summary. It honors the same duck-typed contract as
  # SummaryClient and ThemesClient (call/model_name/max_input_chars) so the
  # SummarizeEvent service can accept a fake in tests.
  #
  ...
  class EventSummaryClient
    ...
    def call(system_prompt:, user_prompt:)
      ...
      raise RequestError, "Event summary model returned invalid JSON: #{error.message}"
    end
    ...
    def post_chat_completion(system_prompt:, user_prompt:)
      ...
      if response.code.to_i >= 400
        ...
        raise RequestError, "Event summary request failed with status #{response.code}: #{message}"
      end

      body
    rescue JSON::ParserError => error
      raise RequestError, "Event summary endpoint returned invalid JSON: #{error.message}"
    end
  end
end
